    ...
    function scanByRawFilter() {
      ...
      function onScan(err, data) {
        if (err) {
          ...
          items = items.concat(data.Items.map(function (item) {
            var model;
          ...
          if (typeof data.LastEvaluatedKey != "undefined" && (options.all.max === 0 || options.all.delay >= 1)) {
            scanReq.ExclusiveStartKey = data.LastEvaluatedKey;
            return dbClient.scan(scanReq, onScan);
          } else {
